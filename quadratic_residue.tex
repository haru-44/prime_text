...
となれば、次は$D,\mu$を求める方法を考えるわけだが、ここでは次が正しいとしてアルゴリズムを与える。
...
$0\le k<s$について、$(AD^m)^{2^{s-1-k}}\equiv1\pmod{p}$ならば、$(AD^m)^{2^{s-1-(k+1)}}\equiv\pm1\pmod{p}$である。
特に$-1$のとき、$m'=m+2^k$とすると、$(AD^{m'})^{2^{s-1-(k+1)}}\equiv1\pmod{p}$である。
\end{Prop}

$k=0,m=0$のとき、$(AD^m)^{2^{s-1-k}}\equiv1\pmod{p}$を満たすので、あとは$k$を大きくしながら常に1になるように$m$を書き換えてやればよい。

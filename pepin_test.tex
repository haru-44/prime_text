...
ということは、$\left(\frac{3}{F_k}\right)$が$-1$になればよくて、実際$\left(\frac{F_k}{3}\right)=1$と$(F_k-1)(3-1)$は$4$の倍数であることを平方剰余の相互法則(\rTheo{legendre_quadratic_reciprocity})に適用すると$\left(\frac{3}{F_k}\right)$が$-1$であることが得られる。

ここまで見るとPepinテストの3という基数は、$\left(\frac{3}{F_k}\right)$が$-1$になれば良いだけのものであり、他の適当な値に変更可能である。

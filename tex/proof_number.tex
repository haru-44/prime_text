...
\subsubsection{\rTheo{pocklington_test}の証明}
$p$を$n$の任意の素因数とする。
...
\rProp{element_order}の2より、$\mbox{ord}(a^R) \mid F$が分かる。
一方で2つ目の条件は、$F$より小さい数では$a^R$の位数にはなり得ないということを言っている。
よって、$\mbox{ord}(a^R) = F$である。

\rProp{lagrange_coro}より、$\mbox{ord}(a^R) \mid \varphi(p)$なので、$\mbox{ord}(a^R) = F$および$\varphi(p)=p-1$を代入すれば$F \mid p-1$を得る。

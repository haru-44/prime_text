...
\rAlgo{fibonacci_pseudoprime_test}の実装では実用に堪えないが、コンセプトは理解できると思う。
実用的な実装は、\rAlgo{quadratic_frobenius_test}を参照すること。

\Algo{Fibonacci擬素数テスト}{fibonacci_pseudoprime_test}{c.f., \rAlgo{fibonacci_sequence}, \rAlgo{legendre_symbol}}
...
\Algo{Lucas数列$V_n$(行列版)}{lucas_sequence_matrix}{c.f., \rAlgo{matrix_mul}, \rAlgo{n_times}}
...
それが\IND{Lucas擬素数テスト}{Lucasきそすうてすと}だ。
こちらも効率的な実装は\rAlgo{quadratic_frobenius_test}に譲る。

\Algo{Lucas擬素数テスト}{lucas_sequence_test}{c.f., \rAlgo{legendre_symbol}, \rAlgo{lucas_sequence}}

やはりこの素数判定法も、合成数を誤って「素数」と判定してしまうことがある。
これを\IND{Lucas擬素数}{Lucasきそすう}(Lucas pseudoprime)と呼ぶ。
